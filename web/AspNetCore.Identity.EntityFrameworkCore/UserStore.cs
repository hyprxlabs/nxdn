...
{
    /// <summary>
    /// Initializes a new instance of the <see cref="UserStore{TUser, TRole, TContext}"/> class.
    /// </summary>
    /// <param name="context">The <see cref="DbContext"/>.</param>
    /// <param name="describer">The <see cref="IdentityErrorDescriber"/>.</param>
...
public class UserStore<TUser, TRole, TContext, TKey> :
    UserStore<TUser, TRole, TContext, TKey, IdentityUserClaim<TKey>, IdentityUserPasswordAuth<TKey>, IdentityUserRole<TKey>, IdentityUserLoginProvider<TKey>, IdentityUserToken<TKey>, IdentityRoleClaim<TKey>>
...
{
    /// <summary>
    /// Initializes a new instance of the <see cref="UserStore{TUser, TRole, TContext, TKey}"/> class.
    /// </summary>
    /// <param name="context">The <see cref="DbContext"/>.</param>
    /// <param name="describer">The <see cref="IdentityErrorDescriber"/>.</param>
    public UserStore(TContext context, IdentityErrorDescriber? describer = null)
...
/// <typeparam name="TUserPasswordAuth">The type representing a user password authentication.</typeparam>
...
public class UserStore<TUser, TRole, TContext, [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] TKey, TUserClaim, TUserPasswordAuth, TUserRole, TUserLoginProvider, TUserToken, TRoleClaim> :
    UserStore<TUser, TRole, TContext, TKey, TUserClaim, TUserPasswordAuth, TUserRole, TUserLoginProvider, TUserToken, TRoleClaim, IdentityUserPasskey<TKey>>
    where TUser : IdentityUser<TKey>
    where TRole : IdentityRole<TKey>
    where TContext : DbContext
    where TKey : IEquatable<TKey>
    ...
    where TUserLoginProvider : IdentityUserLoginProvider<TKey>, new()
    where TUserToken : IdentityUserToken<TKey>, new()
    where TRoleClaim : IdentityRoleClaim<TKey>, new()
{
    /// <summary>
    /// Initializes a new instance of the <see cref="UserStore{TUser, TRole, TContext, TKey, TUserClaim, TUserPasswordAuth, TUserRole, TUserLoginProvider, TUserToken, TRoleClaim}"/> class.
    /// </summary>
    /// <param name="context">The <see cref="DbContext"/>.</param>
    /// <param name="describer">The <see cref="IdentityErrorDescriber"/>.</param>
    public UserStore(TContext context, IdentityErrorDescriber? describer = null)
...
public class UserStore<TUser, TRole, TContext, [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] TKey, TUserClaim, TUserPasswordAuth, TUserRole, TUserLoginProvider, TUserToken, TRoleClaim, TUserPasskey> :
    UserStoreBase<TUser, TRole, TKey, TUserClaim, TUserPasswordAuth, TUserRole, TUserLoginProvider, TUserToken, TRoleClaim>,
    ...
    where TUserPasskey : IdentityUserPasskey<TKey>, new()
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="UserStore{TUser, TRole, TContext, TKey , TUserClaim, TUserPasswordAuth, TUserRole, TUserLoginProvider, TUserToken, TRoleClaim, TUserPasskey}"/> class.
    /// </summary>
    ...
    private DbSet<TUser> UsersSet => this.Context.Set<TUser>();

    private DbSet<TRole> Roles => this.Context.Set<TRole>();

    private DbSet<TUserClaim> UserClaims => this.Context.Set<TUserClaim>();

    private DbSet<TUserRole> UserRoles => this.Context.Set<TUserRole>();

    private DbSet<TUserLoginProvider> UserLogins => this.Context.Set<TUserLoginProvider>();

    private DbSet<TUserToken> UserTokens => this.Context.Set<TUserToken>();

    private DbSet<TUserPasswordAuth> UserPasswordAuths => this.Context.Set<TUserPasswordAuth>();
    ...
    public override async Task AddToRoleAsync(TUser user, string normalizedRoleName, CancellationToken cancellationToken = default(CancellationToken))
    {
        ...
        var roleEntity = await this.FindRoleAsync(normalizedRoleName, cancellationToken);
        if (roleEntity == null)
        {
            throw new InvalidOperationException($"Role {normalizedRoleName} does not exist.");
        }

        this.UserRoles.Add(this.CreateUserRole(user, roleEntity));
    }
    ...
    public override async Task RemoveFromRoleAsync(TUser user, string normalizedRoleName, CancellationToken cancellationToken = default(CancellationToken))
    {
        ...
        if (roleEntity != null)
        {
            var userRole = await this.FindUserRoleAsync(user.Id, roleEntity.Id, cancellationToken);
            if (userRole != null)
            {
    ...
    public override async Task<IList<string>> GetRolesAsync(TUser user, CancellationToken cancellationToken = default(CancellationToken))
    {
        ...
        ArgumentNullException.ThrowIfNull(user);
        var userId = user.Id;
        var query = from userRole in this.UserRoles
    ...
    public override async Task<bool> IsInRoleAsync(TUser user, string normalizedRoleName, CancellationToken cancellationToken = default(CancellationToken))
    {
        ...
        var role = await this.FindRoleAsync(normalizedRoleName, cancellationToken);
        if (role != null)
        {
            var userRole = await this.FindUserRoleAsync(user.Id, role.Id, cancellationToken);
    ...
    public override async Task<IList<TUser>> GetUsersInRoleAsync(string normalizedRoleName, CancellationToken cancellationToken = default(CancellationToken))
    {
        ...
        var query = from userrole in this.UserRoles
                    join user in this.Users on userrole.UserId equals user.Id
    ...
    protected override Task<TRole?> FindRoleAsync(string normalizedRoleName, CancellationToken cancellationToken)
        => this.Roles.SingleOrDefaultAsync(r => r.Name == normalizedRoleName, cancellationToken);
    ...
    protected override Task<TUserRole?> FindUserRoleAsync(TKey userId, TKey roleId, CancellationToken cancellationToken)
        => this.UserRoles.FindAsync(new object[] { userId, roleId }, cancellationToken).AsTask();
    ...
    protected override Task<TUser?> FindUserAsync(TKey userId, CancellationToken cancellationToken)
        => this.Users.SingleOrDefaultAsync(u => u.Id.Equals(userId), cancellationToken);
    ...
    protected override Task<TUserLoginProvider?> FindUserLoginAsync(TKey userId, string loginProvider, string providerKey, CancellationToken cancellationToken)
       => this.UserLogins.SingleOrDefaultAsync(userLogin => userLogin.UserId.Equals(userId) && userLogin.LoginProvider == loginProvider && userLogin.ProviderKey == providerKey, cancellationToken);
    ...
    /// <param name="cancellationToken">The <see cref="CancellationToken"/> used to propagate notifications that the operation should be canceled.</param>
    /// <returns>The user login if it exists.</returns>
    protected override Task<TUserLoginProvider?> FindUserLoginAsync(string loginProvider, string providerKey, CancellationToken cancellationToken)
        => this.UserLogins.SingleOrDefaultAsync(userLogin => userLogin.LoginProvider == loginProvider && userLogin.ProviderKey == providerKey, cancellationToken);
    ...
    /// <returns>The created user passkey.</returns>
    ...
    protected override async Task<TUserPasswordAuth> GetUserPasswordAuthAsync(TUser user)
    {
        ...
        if (auth == null)
        {
            auth = new TUserPasswordAuth { UserId = user.Id };
    ...
    {
        return this.UserPasskeys.SingleOrDefaultAsync(userPasskey => userPasskey.CredentialId.SequenceEqual(credentialId), cancellationToken);
    }
