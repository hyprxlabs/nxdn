...
public class DotEnvDocument : IEnumerable<DotEnvNode>,
    IDictionary<string, string>,
    IReadOnlyDictionary<string, string>
{
    private readonly List<DotEnvNode> entries = new List<DotEnvNode>();

    private readonly Dictionary<string, DotEnvEntry> nameValuePairs = new(StringComparer.OrdinalIgnoreCase);
    ...
    string IDictionary<string, string>.this[string name]
    {
        ...
        set
        {
            ...
            {
                entry = new DotEnvEntry(name.AsSpan(), value.AsSpan());
                this.Add(entry);
                return;
            }
    ...
    public void Add(DotEnvNode entry)
    {
    ...
    {
        var entry = new DotEnvEntry(name.AsSpan(), value);
        this.entries.Add(entry);
    ...
    {
        var entry = new DotEnvEntry(name.AsSpan(), value.AsSpan());
        this.entries.Add(entry);
        this.nameValuePairs.Add(entry.Name, entry);
        this.version++;
    }
    ...
    {
        this.version++;
        this.entries.Add(new DotEnvEmptyLine());
    }

    public void AddComment(ReadOnlySpan<char> comment)
    {
        this.version++;
        this.entries.Add(new DotEnvComment(comment));
    }

    public void AddComment(string comment)
    {
        this.version++;
        this.entries.Add(new DotEnvComment(comment.AsSpan()));
    }
    ...
    public IEnumerable<DotEnvEntry> AsNameValuePairEnumerator()
    {
        foreach (var entry in this.entries)
        {
            if (entry is DotEnvEntry nameValuePair)
    ...
    /// <summary>
    /// Copies the <see cref="DotEnvEntry"/> elements of this document to a dictionary.
    /// </summary>
    ...
    public IEnumerator<DotEnvNode> GetEnumerator()
    ...
    public void Insert(int index, DotEnvNode entry)
    {
        if (entry is DotEnvEntry nameValuePair)
        {
            if (this.nameValuePairs.ContainsKey(nameValuePair.Name))
    ...
    public bool Remove(DotEnvNode entry)
    {
    ...
    public bool TryGetNameValuePair(string key, out DotEnvEntry? value)
    {
        value = default;
        if (this.nameValuePairs.TryGetValue(key, out var entry) && entry is not null)
        {
    ...
    {
        private readonly DotEnvDocument document;

        public OrderedKeyCollection(DotEnvDocument document)
        {
        ...
        public sealed class OrderedEnumerator : IEnumerator<string>
        {
            ...
            internal OrderedEnumerator(DotEnvDocument document)
            {
                this.document = document;
            ...
            public bool MoveNext()
            {
                while (true)
                {
                    ...
                    if (entry is DotEnvEntry record)
                    {
    ...
    public sealed class OrderedValueCollection : ICollection<string>
    {
        ...
        internal OrderedValueCollection(DotEnvDocument document)
        {
            this.document = document;
        }
