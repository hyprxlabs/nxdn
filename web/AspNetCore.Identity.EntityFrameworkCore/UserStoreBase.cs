    ...
    where TUserToken : IdentityUserToken<TKey>, new()
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="UserStoreBase{TUser, TKey, TUserClaim, TUserPasswordAuth, TUserLoginProvider, TUserToken}"/> class.
    /// </summary>
    ...
    public virtual async Task SetPasswordHashAsync(TUser user, string? passwordHash, CancellationToken cancellationToken = default(CancellationToken))
    {
        ...
        this.ThrowIfDisposed();
        ArgumentNullException.ThrowIfNull(user);
        var auth = await this.GetUserPasswordAuthAsync(user).ConfigureAwait(false);
        auth.PasswordDigest = passwordHash;
    }
    ...
    public virtual async Task<string?> GetPasswordHashAsync(TUser user, CancellationToken cancellationToken = default(CancellationToken))
    {
        cancellationToken.ThrowIfCancellationRequested();
        this.ThrowIfDisposed();
        ArgumentNullException.ThrowIfNull(user);
        var auth = await this.GetUserPasswordAuthAsync(user).ConfigureAwait(false);
        return auth.PasswordDigest;
    }
    ...
    public virtual async Task<bool> HasPasswordAsync(TUser user, CancellationToken cancellationToken = default(CancellationToken))
    {
        cancellationToken.ThrowIfCancellationRequested();
        var auth = await this.GetUserPasswordAuthAsync(user).ConfigureAwait(false);
        return !string.IsNullOrWhiteSpace(auth.PasswordDigest);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        cancellationToken.ThrowIfCancellationRequested();
        this.ThrowIfDisposed();
        var userLogin = await this.FindUserLoginAsync(loginProvider, providerKey, cancellationToken).ConfigureAwait(false);
        if (userLogin is null)
            return null;

        return await this.FindUserAsync(userLogin.UserId, cancellationToken).ConfigureAwait(false);
    }
    ...
    /// </returns>
    public virtual async Task<bool> GetEmailConfirmedAsync(TUser user, CancellationToken cancellationToken = default(CancellationToken))
    {
        cancellationToken.ThrowIfCancellationRequested();
        this.ThrowIfDisposed();
        ArgumentNullException.ThrowIfNull(user);
    ...
    public virtual async Task SetEmailConfirmedAsync(TUser user, bool confirmed, CancellationToken cancellationToken = default(CancellationToken))
    {
        ...
        ArgumentNullException.ThrowIfNull(user);
        var auth = await this.GetUserPasswordAuthAsync(user).ConfigureAwait(false);
        auth.IsEmailVerified = confirmed;
    }
    ...
    public virtual Task<string?> GetNormalizedEmailAsync(TUser user, CancellationToken cancellationToken = default(CancellationToken))
    {
        ...
        this.ThrowIfDisposed();
        ArgumentNullException.ThrowIfNull(user);
        return Task.FromResult(user.UpcaseEmail);
    }
    ...
    public virtual Task SetNormalizedEmailAsync(TUser user, string? normalizedEmail, CancellationToken cancellationToken = default(CancellationToken))
    {
        ...
        this.ThrowIfDisposed();
        ArgumentNullException.ThrowIfNull(user);
        user.UpcaseEmail = normalizedEmail;
        return Task.CompletedTask;
    }
    ...
    /// </returns>
    public virtual async Task<bool> GetPhoneNumberConfirmedAsync(TUser user, CancellationToken cancellationToken = default(CancellationToken))
    {
        cancellationToken.ThrowIfCancellationRequested();
        this.ThrowIfDisposed();
        ArgumentNullException.ThrowIfNull(user);
        var auth = await this.GetUserPasswordAuthAsync(user).ConfigureAwait(false);
        return auth.IsPhoneNumberVerified;
    }
    ...
    public virtual async Task SetPhoneNumberConfirmedAsync(TUser user, bool confirmed, CancellationToken cancellationToken = default(CancellationToken))
    {
        ...
        ArgumentNullException.ThrowIfNull(user);
        var auth = await this.GetUserPasswordAuthAsync(user).ConfigureAwait(false);
        auth.IsPhoneNumberVerified = confirmed;
    }
    ...
    /// <param name="cancellationToken">The <see cref="CancellationToken"/> used to propagate notifications that the operation should be canceled.</param>
    /// <returns>The <see cref="Task"/> that represents the asynchronous operation.</returns>
    public async virtual Task SetTwoFactorEnabledAsync(TUser user, bool enabled, CancellationToken cancellationToken = default(CancellationToken))
    {
        cancellationToken.ThrowIfCancellationRequested();
        this.ThrowIfDisposed();
        ArgumentNullException.ThrowIfNull(user);
        var auth = await this.GetUserPasswordAuthAsync(user).ConfigureAwait(false);
        auth.IsTwoFactorEnabled = enabled;
    }
    ...
    /// </returns>
    public async virtual Task<bool> GetTwoFactorEnabledAsync(TUser user, CancellationToken cancellationToken = default(CancellationToken))
    {
        cancellationToken.ThrowIfCancellationRequested();
        this.ThrowIfDisposed();
        ArgumentNullException.ThrowIfNull(user);
        var auth = await this.GetUserPasswordAuthAsync(user)
            .ConfigureAwait(false);
        return auth.IsTwoFactorEnabled;
    }
    ...
    public virtual async Task SetTokenAsync(TUser user, string loginProvider, string name, string? value, CancellationToken cancellationToken)
    {
        ...
        var token = await this.FindTokenAsync(user, loginProvider, name, cancellationToken).ConfigureAwait(false);
        if (token == null)
        {
            await this.AddUserTokenAsync(this.CreateUserToken(user, loginProvider, name, value)).ConfigureAwait(false);
        }
        else
        {
            token.Value = value;
    ...
    public virtual async Task RemoveTokenAsync(TUser user, string loginProvider, string name, CancellationToken cancellationToken)
    {
        ...
        ArgumentNullException.ThrowIfNull(user);
        var entry = await this.FindTokenAsync(user, loginProvider, name, cancellationToken).ConfigureAwait(false);
        if (entry != null)
        {
            await this.RemoveUserTokenAsync(entry).ConfigureAwait(false);
        }
    }
    ...
    public virtual Task SetAuthenticatorKeyAsync(TUser user, string key, CancellationToken cancellationToken)
        => this.SetTokenAsync(user, InternalLoginProvider, AuthenticatorKeyTokenName, key, cancellationToken);
    ...
    public virtual async Task<int> CountCodesAsync(TUser user, CancellationToken cancellationToken)
    {
        ...
        ArgumentNullException.ThrowIfNull(user);
        var mergedCodes = await this.GetTokenAsync(user, InternalLoginProvider, RecoveryCodeTokenName, cancellationToken).ConfigureAwait(false) ?? string.Empty;
...
#pragma warning restore SA1625 // Element documentation should not be copied and pasted
    ...
    protected abstract Task<TUserLoginProvider?> FindUserLoginAsync(TKey userId, string loginProvider, string providerKey, CancellationToken cancellationToken);
    ...
    /// <param name="cancellationToken">The <see cref="CancellationToken"/> used to propagate notifications that the operation should be canceled.</param>
    /// <returns>The user login if it exists.</returns>
    protected abstract Task<TUserLoginProvider?> FindUserLoginAsync(string loginProvider, string providerKey, CancellationToken cancellationToken);
    ...
    /// <returns>The new user claim.</returns>
    ...
    /// <summary>
    /// Called to create a new instance of a <see cref="IdentityUserLoginProvider{TKey}"/>.
    /// </summary>
    /// <param name="user">The associated user.</param>
    /// <param name="login">The associated login.</param>
    /// <returns>The user login provider.</returns>
    protected virtual TUserLoginProvider CreateUserLogin(TUser user, UserLoginInfo login)
    {
        return new TUserLoginProvider
        {
            UserId = user.Id,
    ...
    protected abstract Task<TUserPasswordAuth> GetUserPasswordAuthAsync(TUser user);
}
...
public abstract class UserStoreBase<TUser, TRole, [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] TKey, TUserClaim, TUserPasswordAuth, TUserRole, TUserLogin, TUserToken, TRoleClaim> :
...
{
    /// <summary>
    /// Initializes a new instance of the <see cref="UserStoreBase{TUser, TRole, TKey, TUserClaim, TUserPasswordAuth, TUserRole, TUserLogin, TUserToken, TRoleClaim}"/> class.
    /// </summary>
    /// <param name="describer">The <see cref="IdentityErrorDescriber"/> used to describe store errors.</param>
    public UserStoreBase(IdentityErrorDescriber describer)
    : base(describer)
